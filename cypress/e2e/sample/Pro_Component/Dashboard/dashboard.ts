export class dashBoard {
    ...
    verifyAcademy() {
        cy.get(this.db_profileName)
            .contains('Dartle Official')
    }

    verifyAcademyinfo() {
        cy.get(this.db_academyEmail)
            .contains('support@dartle.app')
    ...
    navigatetoAddmember() {
        ...
        cy.url().should('eq', 'https://academy.dartle.app/members/list/add_player?type=Player');
    }

    navigatetoAddgroup() {
        this.clickAddGroup()
        // Verify that the URL is correct after navigation
        cy.url().should('eq', 'https://academy.dartle.app/groups/add');
    }

    navigatetoAddcoach() {
        this.clickAddcoach()
        // Verify that the URL is correct after navigation
        cy.url().should('eq', 'https://academy.dartle.app/members/list/add_coach?type=Coach');
    }
}