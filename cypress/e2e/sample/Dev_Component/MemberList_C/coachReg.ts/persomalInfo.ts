...
export class coachPersonalInfo {
    ...
    //*********Visit URLs***********
    visitPendinginvites() {
        cy.visit('https://academy.dartle.app/members/list/add_coach', { failOnStatusCode: false });
    ...
    // selectDate(startYear: number, startMonth: string, startDay: number) {
    //     // Open the date picker
    //     cy.get(this.pI_calendar).click();
    ...
    //     // Check if the desired year is greater than 2015
    //     if (startYear > 2015) {
    //         // Click to go to the next year list
    //         cy.get('.mat-calendar-next-button').click(); // Adjust selector based on your actual button
    //     }
    ...
    underageWarning() {
        cy.get(this.underageToaster)
            ...
            .and('contain.text', 'Coach must be atleast 18 years old.');
